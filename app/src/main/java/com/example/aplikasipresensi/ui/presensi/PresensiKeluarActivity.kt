...
class PresensiKeluarActivity : AppCompatActivity(), OnMapReadyCallback {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        context = this@PresensiKeluarActivity
    ...
    private fun saveData() {
        ...
        mApiService.uploadImage("Bearer ${token}", image)?.enqueue(object : Callback<JsonObject?> {
            ...
            override fun onFailure(call: Call<JsonObject?>, t: Throwable) {
                ...
                Toast.makeText(context, "Salah Disini", Toast.LENGTH_SHORT).show()
            }
        })
    }

    private fun absenKeluar(foto: String) {
        val apiConfig = ApiConfig()
        val mApiService: ApiService = apiConfig.getApiService()
        ...
        mApiService.absenKeluar("Bearer ${token}", "2", foto, koordinate, binding.etKeterangan.text.toString())?.enqueue(object :
            ...
            override fun onFailure(call: Call<JsonObject?>, t: Throwable) {
                loading.dismiss()
                Log.d("onfailure : ", t.message!!)
                Toast.makeText(context, "Ga bisa", Toast.LENGTH_SHORT).show()
            }
        })
    }
